...
<div id="container" class="so_main">
    ...
    <div class="Toolbar_inbox">
        ...
        <a onclick="delHuiyuanXq()" class="btn_a"><span>删除</span></a>
    </div>
    <!-- END TOOLBAR -->
    ...
    <div class="list" id='list'>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <#list list.list as huiyuan>
                <tr overstyle="on" id="tr1">
                    <td><input type="checkbox" value="${huiyuan.id}" name="checkbox"></td>
                    <td >${huiyuan.id}</td>
                    <td >${huiyuan.name}</td>
                    <td >${huiyuan.grade}</td>
                    <td >${huiyuan.mouthprice}</td>
                    <td >${huiyuan.yearprice}</td>
                    <td >
                        <#if huiyuan.status==1>
                    ...
                    <td >
                        <a href="/huiyuandengji/toupdHuiyuanXq?id=${huiyuan.id}">编辑</a> |
                        <a onclick="updStatus(${huiyuan.id},${huiyuan.status})">
                            <#if huiyuan.status==1>
                                禁用
                            <#else >
                                启用
...
<script type="text/javascript">
    function updStatus(id,status){
        $.post("/huiyuandengji/updStatus",{id:id,status:status},function (data) {
            alert(data.message)
            location.href="/huiyuandengji/findXqHuiyuandengji";
        })
    }
    function delHuiyuanXq() {
        ids='';
        ...
        ids= ids.substr(1);
        if ( ids!= null && ids!="") {
            $.post("/huiyuandengji/delHuiyuanXqs",{ids:ids},function (data) {
                alert(data.message)
                location.href="/huiyuandengji/findXqHuiyuandengji";
            })
        }else {
