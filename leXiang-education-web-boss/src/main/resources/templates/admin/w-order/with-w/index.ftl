...
<div id="container" class="so_main">
    ...
    <div class="form2" id='search_form' >
        <form action="/queryFin?withdrawstatus=0" method="post">
    ...
    <div class="Toolbar_inbox">
        ...
        <a onclick="admin.fold('search_form')" class="btn_a"><span>搜索</span></a><a onclick="updStatus()" class="btn_a"><span>批量通过</span></a>
    </div>
    <!-- END TOOLBAR -->
    ...
    <div class="list" id='list'>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <th class="line_l"   >ID</th>				<th class="line_l"   >用户</th>				<th class="line_l"   >提现金额</th>				<th class="line_l"   >提现类型</th>				<th class="line_l"   >提现人</th>				<th class="line_l"   >提现账户</th>				<th class="line_l"   >申请时间</th>				<th class="line_l"   >状态</th>				<th class="line_l"   style="width:auto;" >操作</th>		</tr>
            ...
            <tr>
                <td style="width:30px;">
                    <input type="checkbox" name='checkbox' onclick="admin.checkon(this)" value=${w.widthdrawid}>
                        ...
                        <td>${w.widthdrawid}</td>
                        <td>
                            <a event-node="face_card" uid=${w.widthdrawid} href="/queryUserByName" class="" target="_blank">${w.username}</a>
                        </td>
                        ...
                        <td>${w.wcreatdate}</td>
                        <td>
                            <#if w.withdrawstatus=0>待处理<#else>处理中</#if>
                        </td>
...
//表格样式
    ...
    delBankByIds = function () {
        ...
        if ( ids!= null && ids!="") {
            $.post("/delWithById",{ids:ids},function (data) {
                alert(data.message)
                location.href="/queryFin?withdrawstatus=0";
            })
        }else {
            alert("请选择要删除的数据！！")
        }
    ...
    updStatus = function () {
        ...
        ids= ids.substr(1);
        if ( ids!= null && ids!="") {
            $.post("/updWithById",{ids:ids},function (data) {
                alert(data.message)
                location.href="/queryFin?withdrawstatus=0";
            })
        }else {
