...
<div id="container" class="so_main">
    ...
    <div id='page_config' class = "form2 list" >
        ...
        <form action="http://try.51eduline.com/index.php?app=admin&mod=Index&act=savePageConfig" method="POST">
            <input type="hidden" name='pageKey' value="admin_Single_index"  class="s-txt"/>
            ...
            <table width="100%" cellspacing="0" cellpadding="0" border="0">
                ...
                <tr overstyle="on">
                    ...
                    <td width="20%"> <input type="hidden" name='key[]' value='key'> key</td>
                    <td width="30%"><input type="text" name='key_name[]' value='唯一标识'  class="s-txt" style="width:200px" onblur="this.className='s-txt'" onfocus="this.className='s-txt-focus'" /></td>
                    <td width="20%">
                        <select name="key_hidden[]">
                            <option value="0" selected="selected">显示</option>
                            <option value="1" >隐藏</option>
    ...
    <div id='search_config' class = "form2 list" >
        ...
        <form action="http://try.51eduline.com/index.php?app=admin&mod=Index&act=saveSearchConfig" method="POST">
            <input type="hidden" name='searchPageKey' value="S_admin_Single_index" />
            <input type="hidden" name='pageTitle' value="列表" />
            <table width="100%" cellspacing="0" cellpadding="0" border="0">
                <tr>
                    <th>字段</th>
    ...
    <div class="form2" id='search_form' >
        <form action="http://try.51eduline.com/index.php?app=admin&mod=Single&act=index&dosearch=1" method="post">
            <div class="page_btm">
    ...
    <div class="Toolbar_inbox">
        ...
        <a onclick="delPageByIds()" class="btn_a"><span>删除</span></a>
    </div>
    <!-- END TOOLBAR -->
    ...
    <div class="list" id='list'>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
                ...
                <th class="line_l">唯一标识</th>
                <th class="line_l" style="width:auto;" >操作</th>
            </tr>
            <#list page.list as page>
            <tr overstyle="on" id="tr8">
                <td><input type="checkbox" value="${page.id}" name="checkbox"></td>
                <td >${page.id}</td>
                <td ><a target="_blank" href="http://try.51eduline.com/single/8.html">${page.title}</a></td>
                <td >${page.identify}</td>
                <td ><a href="/page/toupdXqPage?id=${page.id}">编辑</a> - <a onclick="delPageById(${page.id})">删除</a></td>
            </tr>
            </#list>
        </table>
    </div>
    ...
    <!-- START BOTTOMBAR -->
    <div class="Toolbar_inbox">
        <div class="page right"><span>共${page.totalPage}条</span></div>
        <!-- 	<a onclick="admin.delSingle()" class="btn_a"><span>删除</span></a> -->
    </div>
    <!-- END BOTTOMBAR -->
</div>
<script>
...
<script type="text/javascript">
    function delPageById(id){
        $.post("/page/delPageById",{id:id},function (data) {
            alert(data.message)
            location.href="/page/findPage";
        })
    }
    function delPageByIds() {
        ids='';
        ...
        ids= ids.substr(1);
        if ( ids!= null && ids!="") {
            $.post("/page/delPageByIds",{ids:ids},function (data) {
                alert(data.message)
                location.href="/page/findPage";
            })
        }else {
