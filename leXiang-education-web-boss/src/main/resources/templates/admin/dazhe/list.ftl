...
<div id="container" class="so_main">
    ...
    <div class="tit_tab">
        <ul>
            <li><a href="http://try.51eduline.com/index.php?app=classroom&mod=AdminDiscount&act=index&tabHash=index" class="on">列表</a></li><li><a href="/dazhe/toaddDazhe" >添加</a></li>
            <!-- 列表 -->
        </ul>
    </div>
    <!-- END TAB框 -->
    ...
    <div id='page_config' class = "form2 list" >
        ...
        <form action="http://try.51eduline.com/index.php?app=admin&mod=Index&act=savePageConfig" method="POST">
            <input type="hidden" name='pageKey' value="classroom_AdminDiscount_index"  class="s-txt"/>
    ...
    <div id='search_config' class = "form2 list" >
        ...
        <form action="http://try.51eduline.com/index.php?app=admin&mod=Index&act=saveSearchConfig" method="POST">
            <input type="hidden" name='searchPageKey' value="S_classroom_AdminDiscount_index" />
            <input type="hidden" name='pageTitle' value="列表" />
            <table width="100%" cellspacing="0" cellpadding="0" border="0">
                <tr>
                    <th>字段</th>
    ...
    <div class="form2" id='search_form' >
        <form action="http://try.51eduline.com/index.php?app=classroom&mod=AdminDiscount&act=index&dosearch=1" method="post">
            ...
            <dl class="lineD" id='dl_course_type'>
                ...
                <dd>
                    ...
                    <select name="course_type" id="form_course_type"  class='s-select' style="width:200px">
    ...
    <div class="Toolbar_inbox">
        ...
        <a onclick="admin.fold('search_form')" class="btn_a"><span>搜索</span></a><a onclick="delDazheXqs()" class="btn_a"><span>删除</span></a>
    </div>
    <!-- END TOOLBAR -->
    ...
    <div class="list" id='list'>
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            <#list list.list as dazhe>
                <tr overstyle="on" id="tr9">
                    <td><input type="checkbox" value="${dazhe.id}"  name="checkbox"></td>
                    <td >${dazhe.id}</td>
                    <td ><a href='http://try.51eduline.com/live/119.html?is_look=1' style='color:#3b5999; ' target='_blank'>${dazhe.playname}</a></td>
                    <td >
                        <#if dazhe.leixing=0>
                            点播课
                        <#elseif dazhe.leixing=1>
                            班级课
                        <#elseif dazhe.leixing=2>
                            直播课
                        <#elseif dazhe.leixing=3>
                            线下课
                        </#if>
                    </td>
                    <td >${dazhe.discounted}折</td>
                    <td ><#if dazhe.status=1>
                            正常
                        <#else >
                            禁用
                        </#if></td>
                    <td >${dazhe.starttime?datetime}</td>
                    <td >${dazhe.endtime?datetime}</td>
                    <td >${dazhe.createtime?datetime}</td>
                    <td ><a href="/dazhe/findXqDazheOne?id=${dazhe.id}">编辑</a> | <a onclick="delDazheXq(${dazhe.id})">删除</a></td>
                </tr>
            </#list>
        </table>
    </div>
...
<script type="text/javascript">
    function delDazheXq(id){
        $.post("/dazhe/delDazheXq",{id:id},function (data) {
            alert(data.message)
            location.href="/dazhe/findXqDazhe";
        })
    }
    function delDazheXqs() {
        ids='';
        ...
        ids= ids.substr(1);
        if ( ids!= null && ids!="") {
            $.post("/dazhe/delDazheXqs",{ids:ids},function (data) {
                alert(data.message)
                location.href="/dazhe/findXqDazhe";
            })
        }else {
